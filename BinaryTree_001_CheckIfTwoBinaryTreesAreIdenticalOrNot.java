public class BinaryTree_001_CheckIfTwoBinaryTreesAreIdenticalOrNot {
    /*
    There is to way to solve this problem which are iterative and recursive ways.

    Recursive Solution
    The idea is to traverse both trees and compare values at their root node. If the value matches, recursively check
    if the first tree's left subtree is identical to the left subtree of the second tree and the right subtree of the
    first tree is identical to the right subtree of the second tree. If value at their root node differs, the trees
    violate data property. If at any point in the recursion, the first tree is empty and the second tree is non-empty,
    or the second tree is empty and the first tree is non-empty, the trees violate structural property, and they cannot
    be identical.

    Iterative Solution
    We use the stack data structure similar to the implicit recursive stack.
     */
    ...
    static class Main {
        ...
        public static boolean isIdenticalRecursively(Node x, Node y) {
            ...
            return (x != null && y != null) && (x.key == y.key) &&
                    isIdenticalRecursively(x.left, y.left) &&
                    isIdenticalRecursively(x.right, y.right);
        }
